...
class SoftDeletionAdmin(ModelAdmin):
    ...
    def soft_delete(self, request: HttpRequest, queryset: QuerySet) -> None:
        queryset.update(is_active=False)
        messages.success(request, Messages.DELETE_COMPLETE.value)

    def restore(self, request: HttpRequest, queryset: QuerySet) -> None:
        queryset.update(is_active=True)
        messages.success(request, Messages.RESTORE_COMPLETE.value)
